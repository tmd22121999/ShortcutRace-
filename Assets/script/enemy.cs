        ...
        //if(speed==0)
            foreach(var x in fov.visibleTargets)
                if(x!=null) {
                    ...
                    if( (x.tag == "Player") && (!x.GetComponent<player>().isHit) && (canKill) && (brickCount>1)&& (x.GetComponent<player>().brickCount>2) && (rand < (x.GetComponent<player>().brickCount-brickCount)*0.015f)){
